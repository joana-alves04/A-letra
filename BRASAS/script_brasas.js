(function () {
    if (window.scriptBrasasCarregado) return;
    ...
    function iniciarJogoBrasas() {
        try {
            ...
            // Variáveis da primeira slide
            const palavraMisterio = container.querySelector('#palavra-misterio');
            const botaoDescobrir = container.querySelector('#botao-descobrir');

            // --- LÓGICA DO CLIQUE INICIAL (Desbloqueio e Desfoque) ---
            if (botaoDescobrir && palavraMisterio) {
                botaoDescobrir.addEventListener('click', () => {
                    palavraMisterio.classList.remove('desfocado');
                    palavraMisterio.classList.add('nitido');

                    botaoDescobrir.innerHTML = '&darr; DESCE PARA CONTINUAR &darr;';
                    botaoDescobrir.classList.remove('interativo');
                    botaoDescobrir.style.cursor = 'default';
                    // Como houve clique, o navegador regista a intenção do utilizador!
                }, { once: true });
            }
            ...
            scrollContainer.addEventListener('scroll', () => {
                ...
                if (progressoScroll > 0.85) {
                    ...
                    if (tvVideo && tvVideo.paused) {
                        let playPromise = tvVideo.play();
                        if (playPromise !== undefined) {
                            playPromise.catch(() => { /* Ignora bloqueio silencioso do navegador */ });
                        }
                    }
                } else {
                    chegouASala = false;
            ...
            console.error("Erro no Jogo das Brasas:", e);
        }
    }
    ...
    function iniciarAbasSobre() {
        try {
            ...
            const canais = [
                ...
                { titulo: "Significado", texto: "Significa dormir uma sesta curta, fechar os olhos ou dormitar durante alguns minutos." },
    ...
    // ==================================================================
    // ROTEADOR CENTRAL E INICIALIZAÇÃO
    // ==================================================================
    function roteador() {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', roteador);
    } else {
        roteador();
    }
